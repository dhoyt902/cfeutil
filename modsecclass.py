...
class CFEsecclass:
    ...
    def Getclasses(self):
        fclass = open('/tmp/secclasses.cf', 'r')
        clslist = []
    ...
    def Isdupe(self, srvname):
        ...
        srvlist = CFEsecclass(classname).Formtonorm()
        x = self.srvname in srvlist
        return x

    def Formtonorm(self):
        temp1 = open('/tmp/temp1.txt', 'w+')
        for i in CFEsecclass(classname=self.classname).Getservers():
            print(i, file=temp1)
        normlist = []
        temp1.close()
    ...
    def Addtoclass(self, srvname):
        ...
        dataset = CFEsecclass(classname).Getservers()[0].split('\n')
        for i in dataset:
            dataset[dataset.index(i)] = re.sub('\n', '', i)
        if CFEsecclass(classname).Isdupe(srvname) is False:
            srvname = CFEsecclass(classname).Normtoform(srvname)
            ...
            wholefile = open('/tmp/secclasses.cf', 'r')
            wholelist = []
            for line in wholefile:
                wholelist.append(line)
            clslist = CFEsecclass().Getclasses()
            ...
            wholefile.close()
            with open('/tmp/secclasses.cf', 'w+') as \
            newfile:
    ...
    def Delfromclass(self, srvname):
        ...
        for i in dataset:
            dataset[dataset.index(i)] = re.sub('\n', '', i)
        if CFEsecclass(classname).Isdupe(srvname) is True:
            srvname = CFEsecclass(classname).Normtoform(srvname)
            for i in dataset:
                if i.isspace():
                    dataset.remove(i)
            ...
            for i in dataset:
                if i != '':
                    dataset[dataset.index(i)] += '\n'
            dataset.append(template[1])
            wholefile = open('/tmp/secclasses.cf',
                'r')
            wholelist = []
            for line in wholefile:
                wholelist.append(line)
